...
type MenteeData = {
  ...
  totalWeeks: number
  fit: number
  tasks: { id: string; title: string; description?: string; status: string; dueLabel?: string; mentorNote?: string }[]
}
...
function TaskRow({
  ...
  task: { id: string; title: string; description?: string; status: string; dueLabel?: string; mentorNote?: string }
  mentorName: string
  onStatus: (status: TaskStatus) => void
  onEditTask: (title: string, description: string) => void
  onComment: (note: string) => void
}) {
  const [mode, setMode] = useState<'none' | 'task' | 'comment'>('none')
  const [titleDraft, setTitleDraft] = useState(task.title)
  const [descDraft, setDescDraft] = useState(task.description ?? '')
  const [noteDraft, setNoteDraft] = useState(task.mentorNote ?? '')
  const hasComment = Boolean(task.mentorNote && task.mentorNote.trim())
  ...
  return (
    <div className="rounded-[2px] border border-line bg-white px-4 py-3.5">
      <div className="flex items-start justify-between gap-3">
        {mode === 'task' ? (
          <div className="flex flex-1 flex-col gap-2">
            ...
            <Textarea rows={2} value={descDraft} onChange={(e) => setDescDraft(e.target.value)} placeholder="Task description — what the mentee needs to do." />
            <div className="flex gap-2">
              <Button size="sm" disabled={titleDraft.trim().length < 2} onClick={() => { onEditTask(titleDraft.trim(), descDraft.trim()); setMode('none') }}>Save</Button>
              <Button size="sm" variant="ghost" onClick={() => { setTitleDraft(task.title); setDescDraft(task.description ?? ''); setMode('none') }}>Cancel</Button>
            </div>
          </div>
        ) : (
      ...
      {/* Task description (the brief) */}
      {mode !== 'task' && task.description && (
        <p className="mt-1.5 text-xs leading-relaxed text-ink-soft">{task.description}</p>
      )}

      {task.dueLabel && mode !== 'task' && (
        <div className="mt-2">
      ...
      {/* Existing comment (mentor feedback), separate from the description */}
      ...
      {mode === 'none' && (
        <div className="mt-3 flex items-center gap-4 text-xs">
          <button type="button" onClick={() => { setTitleDraft(task.title); setDescDraft(task.description ?? ''); setMode('task') }} className="font-semibold text-ink-faint transition-colors hover:text-ink">Edit task</button>
          {!hasComment && (
        ...
        <div className="flex flex-col gap-3">
          {mentee.tasks.map((t) => (
            <TaskRow
              ...
              onEditTask={(title, description) => void editTask({ taskId: t.id as Id<'tasks'>, title, description })}
